...
def test_target_with_scope_prefix_resolves(tmp_path):
    """[[acmebot: wiki/acmebot-credentials]] is shorthand for the path
    'wiki/acmebot-credentials' (the scope prefix is for the human reader,
    confirming the cross-scope nature of the link)."""
    storage = _storage_with_paths(tmp_path, ["wiki/acmebot-credentials.md"])

    canonical, broken = resolve_target("acmebot: wiki/acmebot-credentials", storage)

    assert broken is False
    assert canonical == "wiki/acmebot-credentials"


def test_target_with_scope_prefix_no_space(tmp_path):
    """[[acmebot:wiki/acmebot-credentials]] (no space after colon) — must also work."""
    storage = _storage_with_paths(tmp_path, ["wiki/acmebot-credentials.md"])
...
def test_target_scope_prefix_with_anchor(tmp_path):
    ...
    canonical, broken = resolve_target("acmebot: wiki/acmebot-credentials#section-x", storage)
