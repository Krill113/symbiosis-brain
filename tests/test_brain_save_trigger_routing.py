...
def _run(prompt, env_extra=None, sb_tmp=None, session='s1'):
    ...
    subprocess.run(['bash', str(HOOK), 'prompt-check'], input=payload, text=True, env=env, capture_output=True)
...
def test_route_turn_excluded_from_sessionstart_rm():
    ...
    try:
        ...
        subprocess.run(['bash', str(HOOK), 'prompt-check'], input=p, text=True, env=env, capture_output=True)
        ctr = pathlib.Path(sb) / 'brain-route-turn-sX'
        assert ctr.read_text().strip() == '1'
        subprocess.run(['bash', str(START)], input=json.dumps({'session_id': 'sX'}), text=True, env=env, capture_output=True)
...
def _run_route(prompt, sb, route_class):
    ...
    r = subprocess.run(['bash', str(HOOK), 'prompt-check'],
        input=json.dumps({'session_id': 'sup', 'prompt': prompt}), text=True, env=env, capture_output=True)
    return r.stdout
